...
function Stoop(){
  ...
  function parseNode(node, viewModel){
    ...
    for(var i = 0; i < node.attributes.length; i++){
      ...
      if (attr.name == 'text') {
        ...
        var attrValue = evaluateAttrText(attr, viewModel);
        node.setAttribute(attr.name.split('-')[1], attrValue);
      } else if (attr.name == 'css') {
        var cssDescriptor = evaluateAttrObject(attr);
  ...
  function evaluateAttrObject(attr){
    var result = eval('__deserialized_stoop_variable__=' + attr.value);
    return result;
  }
